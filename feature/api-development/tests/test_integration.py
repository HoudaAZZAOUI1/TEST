...
class TestAPIIntegration:
    ...
    def test_concurrent_requests(self):
        ...
        def make_request():
            response = client.post("/predict", json={"user_id": 1, "viewed_products": [1, 2, 3]})
    ...
    def test_error_handling_invalid_payload(self):
        ...
        for payload in invalid_payloads:
            ...
            assert response.status_code in [400, 422], f"Expected error status for invalid payload: {payload}"
...
class TestAPIDataValidation:
    ...
    def test_viewed_products_validation(self):
        ...
        response = client.post("/predict", json={"user_id": 1, "viewed_products": [1, 2, 3, 4, 5]})
        assert response.status_code == 200

    def test_empty_viewed_products(self):
        """Test empty viewed_products list"""
        response = client.post("/predict", json={"user_id": 1, "viewed_products": []})
